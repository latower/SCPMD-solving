 ...
 ****************************************************************************************/
float dynamic_vtree_convergence_threshold = 1.0;
SddSize time_limit_lr = 25000000;
SddSize time_limit_rr = 25000000;
SddSize time_limit_sw = 25000000;
float size_limit_lr = 1.20;
float size_limit_rr = 1.20;
float size_limit_sw = 1.20;
int cartesian_product_limit_rr = 1024;
int cartesian_product_limit_sw = 1024;
...
int try_move(char move, Vtree** root, Vtree** child, SddManager* manager) {
  ...
  if(move=='l') {
    ...
    if(sdd_vtree_rotate_right(*root,manager,time_limit_rr,size_limit_rr,cartesian_product_limit_rr)) {
    ...
    if(sdd_vtree_swap(*child,manager,time_limit_sw,size_limit_sw,cartesian_product_limit_sw)) {
...
int find_best_of_12_local_vtrees(char* moves, SddSize* best_size, SddSize* best_count, SddLiteral* best_balance, Vtree** root, Vtree** child, SddManager* manager) {
  ...
  // printf("Finding best of 12 local vtrees\n");
...
Vtree* best_of_24_local_vtrees(Vtree* vtree, SddManager* manager) {
  // printf("Finding best of 24 local vtrees\n");
...
Vtree* mip_vtree_search(Vtree* vtree, SddManager* manager) {
  ...
  mip_initialize_vtree_search_state(vtree);
  ...
  do {
    ...
    // printf ("Prev size: %ld Cur_size: %ld Init size: %ld Reduction: %f\n", 
            // prev_size, cur_size, init_size, reduction);
    prev_size = cur_size;
...
void sdd_manager_mip_minimize ( SddManager *manager ) {
  printf("sdd_manager_mip_minimize is called\n");
  mip_initialize_vtree_search_state(sdd_manager_vtree(manager));
  mip_vtree_search ( sdd_manager_vtree(manager), manager );
  // sdd_manager_set_minimize_function ( mip_vtree_search, manager );
  // sdd_manager_minimize(manager);
  printf("done minimizing\n");
}

void sdd_manager_set_mip_minimize(SddManager* manager) {
  printf ( "SETTING MIP MINIMIZATION ALGORITHM\n" );
  sdd_manager_set_minimize_function ( mip_vtree_search, manager );
}

void sdd_manager_manually_set_convergence_threshold(float conv_threshold, SddManager *manager) {
  dynamic_vtree_convergence_threshold = conv_threshold;
  printf("Set convergence_threshold = %f\n", dynamic_vtree_convergence_threshold);
}

void sdd_manager_manually_set_lr_time_limit(SddSize time_limit, SddManager *manager) {
  time_limit_lr = time_limit;
  printf("Set time_limit_lr = %ld\n", time_limit_lr);
}

void sdd_manager_manually_set_rr_time_limit(SddSize time_limit, SddManager *manager) {
  time_limit_rr = time_limit;
  printf("Set time_limit_rr = %ld\n", time_limit_rr);
}

void sdd_manager_manually_set_sw_time_limit(SddSize time_limit, SddManager *manager) {
  time_limit_sw = time_limit;
  printf("Set time_limit_lr = %ld\n", time_limit_sw);
}

void sdd_manager_manually_set_lr_size_limit(float size_limit, SddManager *manager) {
  size_limit_lr = size_limit;
  printf("Set size_limit_lr = %f\n", size_limit_lr);
}

void sdd_manager_manually_set_rr_size_limit(float size_limit, SddManager *manager) {
  size_limit_rr = size_limit;
  printf("Set size_limit_rr = %f\n", size_limit_rr);
}

void sdd_manager_manually_set_sw_size_limit(float size_limit, SddManager *manager) {
  size_limit_sw = size_limit;
  printf("Set size_limit_sw = %f\n", size_limit_sw);
}

void sdd_manager_manually_set_rr_cartesian_product_limit(int cartesian_product_limit, SddManager *manager) {
  cartesian_product_limit_rr = cartesian_product_limit;
  printf("Set cartesian_product_limit_rr = %d\n", cartesian_product_limit_rr);
}

void sdd_manager_manually_set_sw_cartesian_product_limit(int cartesian_product_limit, SddManager *manager) {
  cartesian_product_limit_sw = cartesian_product_limit;
  printf("Set cartesian_product_limit_sw = %d\n", cartesian_product_limit_sw);
}

void sdd_manager_print_parameters(SddManager *manager) {
  printf("MANUALLY FIXED PARAMETERS:\n");
  printf("dynamic_vtree_convergence_threshold = %f\n", dynamic_vtree_convergence_threshold);
  printf("time_limit_lr =                       %ld\n", time_limit_lr);
  printf("time_limit_rr =                       %ld\n", time_limit_rr);
  printf("time_limit_sw =                       %ld\n", time_limit_sw);
  printf("size_limit_lr =                       %f\n", size_limit_lr);
  printf("size_limit_rr =                       %f\n", size_limit_rr);
  printf("size_limit_sw =                       %f\n", size_limit_sw);
  printf("cartesian_product_limit_rr =          %d\n", cartesian_product_limit_rr);
  printf("cartesian_product_limit_sw =          %d\n", cartesian_product_limit_sw);
}

void sdd_manager_reset_parameters(SddManager *manager) {
  dynamic_vtree_convergence_threshold = 1.0;
  time_limit_lr = 25000000;
  time_limit_rr = 25000000;
  time_limit_sw = 25000000;
  size_limit_lr = 1.20;
  size_limit_rr = 1.20;
  size_limit_sw = 1.20;
  cartesian_product_limit_rr = 1024;
  cartesian_product_limit_sw = 1024;
}
