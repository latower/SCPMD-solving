...
class sdd2mip_builder(object):
	def __init__(self,
		...
		#daniel: removed call to set_parameters because its done in run_gurobi.py
	...
	def set_parameters(self, timeout, verbosity, presolve, logfile):
		self.model.Params.OutputFlag = verbosity
		if self.timeout:
	...
	def solve(self):
		...
		if self.status == GRB.OPTIMAL:
			...
			self.nTrue = len([name for name in self.solution if bool(round(self.solution[name])) == True])
	...
	def save_solution(self, filename):
		...
		if self.model.Status == GRB.OPTIMAL:				
			...
			for (name, val) in sorted(self.solution.items(), reverse=True):
				sf.write('{:<30s}{:<s}\n'.format(name, str(bool(round(val)))))
